...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO participants(participant_id, user_id,workshop_id) VALUES (?, ?, ?)");
    
    $stmt->bind_param("iss", $participant_id, $user_id,$workshop_id);
    // Set parameters and execute
    $participant_id = $_POST['book_id'];
...
<body>
    <center><h2>participants Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all participants
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $participant_id = $row['participant_id']; // Fetch the participant_id
        echo "<tr>
            <td>" . $row['participant_id'] . "</td>
            <td>" . $row['user_id'] . "</td>
            <td>" . $row['workshop_id'] . "</td>
            <td><a style='padding:4px' href='delete_participants.php?participant_id=$participant_id'>Delete</a></td> 
            <td><a style='padding:4px' href='update_participants.php?participant_id=$participant_id'>Update</a></td> 
        </tr>";
    }

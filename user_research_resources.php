...
<head>
  ...
  <title>user_research_resources Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
    <h1><u>user_research_resources Form</u></h1>
...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <input type="NUMBER" id="passengeid" name="passengeid" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO user_research_resources(resource_id, title, description,file_url,workshop_id) VALUES (?, ?, ?,?,?)");
    $stmt->bind_param("issss", $resource_id, $title,$description,$file_url,$workshop_id);
    // Set parameters and execute
    $resource_id = $_POST['book_id'];
    $title = $_POST['ride_id'];
    $description = $_POST['passenger_id'];
    $file_url = $_POST['passengerid'];
    $workshop_id = $_POST['passengeid'];
...
<body>
    <center><h2>user_research_resources Table</h2></center>
    <table border="3">
        <tr>
            <th>resource_id</th>
            <th>title</th>
...
// Prepare SQL query to retrieve all user_research_resources
$sql = "SELECT * FROM user_research_resources";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr>
            ...
            <td><a style='padding:4px' href='delete_user_research_resources.php?resource_id=$resource_id'>Delete</a></td> 
            <td><a style='padding:4px' href='update_user_research_resources.php?resource_id=$resource_id'>Update</a></td> 
        </tr>";
    }
